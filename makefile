...
build: $(word) $(phrase) $(spelling)
	...
	# 单字码表
	cat $(word) >> $(out)/ymdz.word.dict.yaml
	$(gen)/comp-gen.py $(word) >> $(out)/ymdz.comp.dict.yaml
	# 空格词码表
	cat $(phrase) >> $(out)/ym.phrases.word.dict.yaml
	$(gen)/comp-gen.py $(phrase) >> $(out)/ym.phrases.comp.dict.yaml
